...
func main() {
	jsonPath := "internal/config/providers.json"
	configDir := "internal/config"
	watcherDir := "internal/watcher"
	...
	if err != nil {
		log.Fatalf("failed to read providers.json: %v", err)
	}
	...
	synthPath := filepath.Join(watcherDir, "synthesizer/synthesizer_generated.go")
	if _, err := os.Stat(filepath.Dir(synthPath)); err == nil {
		generate(synthPath, synthTemplate, templateData, funcMap)
	}

	// Generate diff file
	diffPath := filepath.Join(watcherDir, "diff/diff_generated.go")
	if _, err := os.Stat(filepath.Dir(diffPath)); err == nil {
		generate(diffPath, diffTemplate, templateData, funcMap)
	}
}
