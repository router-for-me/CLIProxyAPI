...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	// === cliproxyapi++ 扩展: 注册 Kiro OAuth Web 路由 ===
	...
	// === cliproxyapi++ extension: Sync provider metrics to SHM bridge ===
	go s.startSHMSyncLoop()
...
// (management handlers moved to pkg/llmproxy/api/handlers/management)
...
// startSHMSyncLoop periodically syncs provider metrics to the shared memory mesh.
func (s *Server) startSHMSyncLoop() {
	shmPath := os.Getenv("THEGENT_SHM_PATH")
	if shmPath == "" {
		shmPath = "/tmp/thegent-bridge/state.shm"
	}

	// Ensure directory exists
	shmDir := filepath.Dir(shmPath)
