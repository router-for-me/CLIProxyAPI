...
func joinByteChunks(chunks [][]byte) string {
...
func TestUseGitHubCopilotResponsesEndpoint_CodexMiniModel(t *testing.T) {
	t.Parallel()
	if !useGitHubCopilotResponsesEndpoint(sdktranslator.FromString("openai"), "gpt-5.1-codex-mini") {
		t.Fatal("expected codex-mini model to use /responses")
	}
}
...
func TestTranslateGitHubCopilotResponsesNonStreamToClaude_TextMapping(t *testing.T) {
	...
	out := string(translateGitHubCopilotResponsesNonStreamToClaude(resp))
...
func TestTranslateGitHubCopilotResponsesStreamToClaude_TextLifecycle(t *testing.T) {
	...
	deltaStrs := make([]string, len(delta))
	for i, d := range delta {
		deltaStrs[i] = string(d)
	}
	...
	completedStrs := make([]string, len(completed))
	for i, c := range completed {
		completedStrs[i] = string(c)
	}
...
func TestTranslateGitHubCopilotResponses_Parity_TextAndToolAcrossStreamModes(t *testing.T) {
	...
	textDelta := joinByteChunks(translateGitHubCopilotResponsesStreamToClaude([]byte(`data: {"type":"response.output_text.delta","delta":"hello parity"}`), &param))
	toolAdded := joinByteChunks(translateGitHubCopilotResponsesStreamToClaude([]byte(`data: {"type":"response.output_item.added","item":{"type":"function_call","call_id":"call_1","name":"sum","id":"fc_1"},"output_index":1}`), &param))
	toolDone := joinByteChunks(translateGitHubCopilotResponsesStreamToClaude([]byte(`data: {"type":"response.function_call_arguments.done","item_id":"fc_1","output_index":1,"arguments":"{\"a\":1}"}`), &param))
