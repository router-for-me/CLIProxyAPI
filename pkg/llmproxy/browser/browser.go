...
var incognitoMode atomic.Bool

// SetIncognitoMode controls whether OpenURL should try to launch an isolated browser session.
func SetIncognitoMode(enabled bool) {
	incognitoMode.Store(enabled)
}
...
func OpenURL(url string) error {
	...
	if incognitoMode.Load() {
		if err := openURLIncognito(url); err == nil {
			log.Debug("Successfully opened URL using incognito browser command")
			return nil
		} else {
			log.Debugf("incognito browser command failed: %v, falling back to default browser", err)
		}
	}
...
func openURLIncognito(url string) error {
	...
	case "darwin":
		candidates = [][]string{
			...
			{"open", "-na", "Chromium", "--args", "--incognito", url},
	...
	default:
		return fmt.Errorf("incognito mode is unsupported on %s", runtime.GOOS)
	}
	...
	return fmt.Errorf("no incognito browser command available")
}

// CloseBrowser is best-effort cleanup for auth flows that opened a browser.
