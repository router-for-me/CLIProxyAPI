...
func TestResponseFormatSchemaIsNormalizedForCodex(t *testing.T) {
	input := []byte(`{
		"model": "gpt-4o",
		"messages": [
	...
	required := gjson.Get(result, "text.format.schema.properties.issues.items.required").Array()
	if len(required) != 2 {
		t.Fatalf("nested required length = %d, want 2; output=%s", len(required), result)
	}
	if !gjson.Get(result, `text.format.schema.properties.issues.items.required.#(=="title")`).Exists() {
		t.Fatalf("nested required missing title; output=%s", result)
	}
	if !gjson.Get(result, `text.format.schema.properties.issues.items.required.#(=="dimension")`).Exists() {
		t.Fatalf("nested required missing dimension; output=%s", result)
	}

	dimensionType := gjson.Get(result, "text.format.schema.properties.issues.items.properties.dimension.type").Array()
	if len(dimensionType) != 2 {
		t.Fatalf("dimension type length = %d, want 2; output=%s", len(dimensionType), result)
	}
	if dimensionType[0].String() != "string" || dimensionType[1].String() != "null" {
		t.Fatalf("dimension type = %s, want [string null]; output=%s", gjson.Get(result, "text.format.schema.properties.issues.items.properties.dimension.type").Raw, result)
	}
}
