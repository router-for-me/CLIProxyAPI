...
// shouldMapClaudeThinkingToGPTReasoning decides whether a Claude-format `thinking` content
// block should be translated into an OpenAI-style `reasoning_content` field when replaying
// conversation history to an OpenAI-compatible upstream.
//
// Blocks without a signature are assumed to originate from OpenAI-compatible providers
// (e.g. DeepSeek, GLM, Kimi via OpenCode Zen); those providers never emit an Anthropic-style
// signature, so their thinking blocks are translated to `reasoning_content` rather than
// dropped. Blocks with a signature are only forwarded if that signature is compatible with
// the GPT provider family; signatures from other providers (Claude-native, Gemini-native)
// are filtered out.
//
// NOTE: in round-robin pools that mix multiple OpenAI-compatible upstreams under one
// client-facing alias, this means a reasoning trace generated by provider A on one turn
// may be forwarded as reasoning_content to a different provider B on a later turn, since
// neither signs its output. This is assumed harmless (inert prior-turn context) but has
// not been validated against every provider's strict-mode behavior.
func shouldMapClaudeThinkingToGPTReasoning(part gjson.Result) bool {
	...
	if !signature.Exists() || strings.TrimSpace(signature.String()) == "" {
		log.WithFields(log.Fields{
			"component": "openai_claude_translator",
			"action":    "forward_thinking_block",
			"reason":    "no_provider_signature",
		}).Debug("openai claude translator: forwarding unsigned thinking block as reasoning_content")
		return true
	}
