...
func TestNextPadIndexAndEmptyTextBlockIndexAfterClose(t *testing.T) {
	...
	if got := EmptyTextBlockIndexAfterClose(0, true); got != 1 {
		t.Fatalf("EmptyTextBlockIndexAfterClose(0, true) = %d, want 1", got)
	}
	if got := EmptyTextBlockIndexAfterClose(1, false); got != 1 {
		t.Fatalf("EmptyTextBlockIndexAfterClose(1, false) = %d, want 1", got)
	}
}
...
func TestInjectReasoningOnlyEmptyText(t *testing.T) {
	sse := AppendSSEEventString(nil, "content_block_start", `{"type":"content_block_start","index":0,"content_block":{"type":"thinking","thinking":""}}`, 3)
	sse = AppendSSEEventString(sse, "content_block_delta", `{"type":"content_block_delta","index":0,"delta":{"type":"thinking_delta","thinking":"reason"}}`, 3)
	sse = AppendSSEEventString(sse, "content_block_stop", `{"type":"content_block_stop","index":0}`, 3)
	sse = AppendSSEEventString(sse, "message_delta", `{"type":"message_delta","delta":{"stop_reason":"end_turn","stop_sequence":null}}`, 3)
	sse = AppendSSEEventString(sse, "message_stop", `{"type":"message_stop"}`, 3)
	...
	truncated := AppendSSEEventString(nil, "content_block_start", `{"type":"content_block_start","index":0,"content_block":{"type":"thinking","thinking":""}}`, 3)
	truncated = AppendSSEEventString(truncated, "message_delta", `{"type":"message_delta","delta":{"stop_reason":"max_tokens","stop_sequence":null}}`, 3)
