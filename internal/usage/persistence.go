...
func SaveToFile(filePath string) {
	...
	log.Infof("usage persistence: about to write %d bytes to %s.tmp", buf.Len(), filePath)
	...
	log.Infof("usage persistence: successfully wrote tmp file, renaming to %s", filePath)
	...
	log.Infof("usage persistence: successfully saved usage to %s", filePath)
}
