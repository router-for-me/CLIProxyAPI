...
func (m configTabModel) parseConfig(cfg map[string]any) []configField {
	...
	fields = append(fields, configField{"Usage Detail Limit", "usage-detail-retention-limit", "int", fmt.Sprintf("%.0f", getFloat(cfg, "usage-detail-retention-limit")), nil})
...
func fieldSection(apiPath string) string {
	...
	case "logging-to-file", "logs-max-total-size-mb", "error-logs-max-files", "usage-statistics-enabled", "usage-detail-retention-limit", "request-log":
