...
func (l *FileRequestLogger) logRequestWithSources(url, method string, requestHeaders map[string][]string, body []byte, statusCode int, responseHeaders map[string][]string, response, websocketTimeline []byte, websocketTimelineSource *FileBodySource, apiRequest []byte, apiRequestSource *FileBodySource, apiResponse []byte, apiResponseSource *FileBodySource, apiWebsocketTimeline []byte, apiWebsocketTimelineSource *FileBodySource, apiResponseErrors []*interfaces.ErrorMessage, force bool, requestID string, requestTimestamp, apiResponseTimestamp time.Time) error {
	...
	settings := l.runtimeSettingsSnapshot()
	if !settings.enabled && !force {
		return nil
	}

	if settings.homeEnabled && settings.enabled {
	...
	if errPublish := publishRequestLog(filePath, func(logFile *os.File) error {
		return l.writeNonStreamingLog(
			logFile,
			url, method, requestHeaders, body, requestBodyPath,
			websocketTimeline, websocketTimelineSource,
			apiRequest, apiRequestSource,
			apiResponse, apiResponseSource,
			apiWebsocketTimeline, apiWebsocketTimelineSource,
			apiResponseErrors, statusCode, responseHeaders, responseToWrite,
			decompressErr, requestTimestamp, apiResponseTimestamp,
		)
	}); errPublish != nil {
	...
	if force && !settings.enabled {
		if errCleanup := l.cleanupOldErrorLogs(settings.errorLogsMaxFiles); errCleanup != nil {
...
// cleanupOldErrorLogs keeps only the newest maxFiles forced error log files.
func (l *FileRequestLogger) cleanupOldErrorLogs(maxFiles int) error {
	if maxFiles <= 0 {
		return nil
	}
