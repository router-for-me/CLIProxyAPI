...
func (e *GeminiExecutor) CountTokens(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		errSummary := helps.SummarizeErrorBody(resp.Header.Get("Content-Type"), data)
		entry := helps.LogWithRequestID(ctx)
		entry.Debugf("request error, error status: %d, error message: %s", resp.StatusCode, errSummary)
		helps.LogUpstreamError(entry, e.Identifier(), req.Model, authID, authLabel, resp.StatusCode, errSummary)
