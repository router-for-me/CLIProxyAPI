...
func TestOpenAICompatExecutorCompactAppliesThinkingSuffix(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
	...
	_, err := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		...
		Payload: []byte(`{"model":"gpt-5.1-codex-max(high)","input":[{"role":"user","content":"hi"}]}`),
	}, cliproxyexecutor.Options{
		SourceFormat: sdktranslator.FromString("openai-response"),
		Alt:          "responses/compact",
		Stream:       false,
	...
	if got := gjson.GetBytes(gotBody, "reasoning.effort").String(); got != "high" {
		t.Fatalf("reasoning.effort = %q, want high in body %s", got, string(gotBody))
	}
}
