...
func TestKimiExecutorResponsesReplaysDiscoveredTools(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "additional current function wins over history", input: output(oldPing) + "," + additional(ping) + "," + user, wantInput: additional(ping) + "," + user},
		{name: "additional current namespace merges new members", input: output(namespace("workspace", oldPing+","+read)) + "," + additional(namespace("workspace", ping)) + "," + user, wantInput: additional(namespace("workspace", ping+","+read)) + "," + user},
		{name: "additional function identity includes namespace", input: output(namespace("remote", oldPing)) + "," + additional(namespace("local", ping)) + "," + user, wantInput: additional(namespace("local", ping)) + "," + user, wantTools: "[" + namespace("remote", oldPing) + "]"},
		{name: "all current channels keep their definitions", input: output(oldPing) + "," + additional(oldPing) + "," + additional(ping) + "," + user, tools: "[" + ping + "]", wantInput: additional(oldPing) + "," + additional(ping) + "," + user, wantTools: "[" + ping + "]"},
		{name: "namespace members declared in separate channels win", input: output(namespace("workspace", oldPing+","+read)) + "," + additional(namespace("workspace", read)) + "," + user, tools: "[" + namespace("workspace", ping) + "]", wantInput: additional(namespace("workspace", read)) + "," + user, wantTools: "[" + namespace("workspace", ping) + "]"},
		{name: "new member goes to first existing namespace", input: output(namespace("workspace", ping+","+read+`,{"type":"function","name":"write","parameters":{"type":"object"}}`)) + "," + additional(namespace("workspace", read)) + "," + user, tools: "[" + namespace("workspace", ping) + "]", wantInput: additional(namespace("workspace", read)) + "," + user, wantTools: "[" + namespace("workspace", ping+`,{"type":"function","name":"write","parameters":{"type":"object"}}`) + "]"},
		{name: "nested discoveries use their existing additional namespace", input: output(namespace("workspace", namespace("remote", oldPing+","+read))) + "," + additional(namespace("workspace", namespace("remote", ping))) + "," + user, tools: "[" + namespace("workspace", ping) + "]", wantInput: additional(namespace("workspace", namespace("remote", ping+","+read))) + "," + user, wantTools: "[" + namespace("workspace", ping) + "]"},
		{name: "successive discoveries in additional namespace use latest", input: output(namespace("workspace", oldPing)) + "," + output(namespace("workspace", ping)) + "," + additional(namespace("workspace", read)) + "," + user, wantInput: additional(namespace("workspace", read+","+ping)) + "," + user},
		{name: "additional discovered schema is normalized", input: output(namespace("workspace", `{"type":"function","name":"read","parameters":{"$defs":{"value":{"type":"string"}},"properties":{"path":{"$ref":"#/$defs/value"}}}}`)) + "," + additional(namespace("workspace", ping)) + "," + user, wantInput: additional(namespace("workspace", ping+`,{"type":"function","name":"read","parameters":{"properties":{"path":{"type":"string"}},"type":"object"}}`)) + "," + user},
		{name: "multiple additional namespaces retain member precedence", input: output(namespace("workspace", oldPing+","+read)) + "," + additional(namespace("workspace", ping)) + "," + additional(namespace("workspace", read)) + "," + user, wantInput: additional(namespace("workspace", ping)) + "," + additional(namespace("workspace", read)) + "," + user},
