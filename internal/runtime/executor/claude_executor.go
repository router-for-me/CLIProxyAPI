...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	body, originalTranslated := helps.TranslateRequestWithOriginal(e.cfg, from, to, baseModel, req.Payload, originalPayload, stream)
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		b, readErr := helps.ReadErrorResponseBody(errBody)
		if readErr != nil {
	...
	data, err := helps.ReadNonStreamResponseBody(decodedBody)
	if err != nil {
		helps.RecordAPIResponseError(ctx, e.cfg, err)
		return resp, err
	}
