...
func restoreClaudeOAuthToolResponse(body []byte, auth *cliproxyauth.Auth, apiKey string, reverseAliasMap map[string]string) []byte {
	if !isClaudeOAuthToken(apiKey) || auth.ToolPrefixDisabled() {
		return body
	}
	if len(reverseAliasMap) > 0 {
		return stripClaudeToolAliasesFromResponse(body, reverseAliasMap)
	}
...
func restoreClaudeOAuthToolStreamLine(line []byte, auth *cliproxyauth.Auth, apiKey string, reverseAliasMap map[string]string) []byte {
	if !isClaudeOAuthToken(apiKey) || auth.ToolPrefixDisabled() {
		return line
	}
	if len(reverseAliasMap) > 0 {
		return stripClaudeToolAliasesFromStreamLine(line, reverseAliasMap)
	}
...
// prepareUpstreamBody applies OAuth effort injection and tool aliasing for upstream requests.
func (e *ClaudeExecutor) prepareUpstreamBody(ctx context.Context, body []byte, auth *cliproxyauth.Auth, apiKey string) ([]byte, map[string]string) {
	...
	return bodyForUpstream, claudeToolAliasReverse
}
...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	oauthToken := isClaudeOAuthToken(apiKey)
	clientSource := detectOAuthClientSource(getClientUserAgent(ctx))
	bodyForUpstream, claudeToolAliasReverse := e.prepareUpstreamBody(ctx, body, auth, apiKey)
	...
	var clientSourceOpts []oauthClientSource
	if oauthToken {
		clientSourceOpts = []oauthClientSource{clientSource}
	}
	applyClaudeHeaders(httpReq, auth, apiKey, false, extraBetas, e.cfg, clientSourceOpts...)
...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	var clientSourceOpts []oauthClientSource
	if oauthToken {
		clientSourceOpts = []oauthClientSource{clientSourceStream}
	}
	applyClaudeHeaders(httpReq, auth, apiKey, true, extraBetas, e.cfg, clientSourceOpts...)
	var authID, authLabel, authType, authValue string
	if auth != nil {
		authID = auth.ID
		authLabel = auth.Label
...
func (e *ClaudeExecutor) CountTokens(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	if oauthTokenCT {
		...
		clientSourceOpts = []oauthClientSource{clientSourceCT}
	}
	applyClaudeHeaders(httpReq, auth, apiKey, false, extraBetas, e.cfg, clientSourceOpts...)
	var authID, authLabel, authType, authValue string
	if auth != nil {
...
func stripClaudeToolAliasesFromResponse(body []byte, reverseAliasMap map[string]string) []byte {
	if len(reverseAliasMap) == 0 {
		return body
	}
	content := gjson.GetBytes(body, "content")
	...
	content.ForEach(func(index, part gjson.Result) bool {
		...
		case "tool_use":
			name := part.Get("name").String()
			if original, ok := reverseAliasMap[name]; ok && original != "" {
				path := fmt.Sprintf("content.%d.name", index.Int())
				body, _ = sjson.SetBytes(body, path, original)
			}
		case "tool_reference":
			toolName := part.Get("tool_name").String()
			if original, ok := reverseAliasMap[toolName]; ok && original != "" {
				path := fmt.Sprintf("content.%d.tool_name", index.Int())
				body, _ = sjson.SetBytes(body, path, original)
			}
		case "tool_result":
			...
			if nestedContent.Exists() && nestedContent.IsArray() {
				nestedContent.ForEach(func(nestedIndex, nestedPart gjson.Result) bool {
					...
					nestedToolName := nestedPart.Get("tool_name").String()
					if original, ok := reverseAliasMap[nestedToolName]; ok && original != "" {
						nestedPath := fmt.Sprintf("content.%d.content.%d.tool_name", index.Int(), nestedIndex.Int())
...
func stripClaudeToolAliasesFromStreamLine(line []byte, reverseAliasMap map[string]string) []byte {
	if len(reverseAliasMap) == 0 {
		return line
	}
	...
	case "tool_use":
		...
		original, ok := reverseAliasMap[name]
		if !ok || original == "" {
			return line
		}
	...
	case "tool_reference":
		...
		original, ok := reverseAliasMap[toolName]
		if !ok || original == "" {
			return line
		}
...
func checkSystemInstructionsWithSigningMode(payload []byte, strictMode bool, experimentalCCHSigning bool, oauthMode bool, version, entrypoint, workload string) []byte {
	...
	systemBlocks := []json.RawMessage{
		json.RawMessage(billingBlock),
		json.RawMessage(agentBlock),
		json.RawMessage(introBlock),
		json.RawMessage(systemBlock),
		json.RawMessage(doingTasksBlock),
		json.RawMessage(actionsBlock),
		json.RawMessage(usingToolsBlock),
		json.RawMessage(toneAndStyleBlock),
		json.RawMessage(outputEfficiencyBlock),
	}
	systemResult, err := json.Marshal(systemBlocks)
	if err != nil {
		log.Errorf("claude: failed to marshal system blocks: %v", err)
		return payload
	}
	payload, _ = sjson.SetRawBytes(payload, "system", systemResult)
...
func applyCloaking(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, payload []byte, model string, apiKey string) []byte {
	...
	if !helps.ShouldCloak(cloakMode, clientUserAgent) {
		log.Debugf("claude: cloaking skipped (mode=%s, ua=%q)", cloakMode, clientUserAgent)
		return payload
	}

	log.Debugf("claude: cloaking applied (mode=%s, oauth=%v, strict=%v, model=%s, ua=%q)", cloakMode, oauthToken, strictMode, model, clientUserAgent)
