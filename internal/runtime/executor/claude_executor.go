...
func (e *ClaudeExecutor) executeClaudeMessagesHTTPRequest(
	...
	for requestAttempt := 0; requestAttempt < 2; requestAttempt++ {
		...
		recordClaudeUpstreamRequest(ctx, e.cfg, auth, requestURL, httpRequest, requestBody)
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if decErr != nil {
			...
			return cliproxyexecutor.Response{}, statusErrWithHeaders{
				statusErr: statusErr{code: resp.StatusCode, msg: msg},
		...
		return cliproxyexecutor.Response{}, statusErrWithHeaders{
			statusErr: statusErr{code: resp.StatusCode, msg: string(b)},
			headers:   resp.Header.Clone(),
		}
	}
...
func applyClaudeHeadersWithSession(r *http.Request, auth *cliproxyauth.Auth, apiKey string, stream bool, extraBetas []string, resolvedSessionID string, cfg *config.Config) error {
	...
	if effectiveSessionID != "" {
		...
		misc.EnsureHeader(r.Header, ginHeaders, "X-Claude-Code-Session-Id", fallbackSessionID)
	}
