...
func (s *codexWebsocketSession) setActive(conn *websocket.Conn, ch chan codexWebsocketRead) {
	...
	s.activeConn = conn
	s.activeCh = ch
	s.activeAccept = conn != nil && ch != nil
	if s.activeAccept {
		...
		if sess != nil {
			...
			for sendRetryAttempt := 0; ; sendRetryAttempt++ {
				...
				if errSendRetry := writeCodexWebsocketMessage(sess, conn, wsReqBodyRetry); errSendRetry == nil {
					...
					if shouldRetryCodexWebsocketSendError(sess, conn, errSendRetry, upstreamBody, sendRetryAttempt) && !codexWebsocketRequestCannotRetryFresh(sess, conn, upstreamBody) {
						sess.suppressReadErrorForConn(conn)
						sess.stopActiveDelivery(readCh)
						e.clearUpstreamConn(sess, conn, "send_retry_close_sent", errSendRetry, false)
						readCh = nil
						helps.RecordAPIWebsocketError(ctx, e.cfg, "send_retry_close_sent", errSendRetry)
						continue
					}
					if shouldFallbackCodexWebsocketSendErrorToHTTP(sess, conn, errSendRetry, upstreamBody) {
						e.clearUpstreamConn(sess, conn, "send_close_sent_http_fallback", errSendRetry, false)
	...
	for {
		...
		if errRead != nil {
			...
			if sess != nil && !receivedPayload && shouldRetryCodexWebsocketPrePayloadReadError(errRead, readRetryAttempt) && !codexWebsocketRequestCannotRetryFresh(sess, conn, upstreamBody) {
				...
				if errSendRetry := writeCodexWebsocketMessage(sess, conn, wsReqBodyRetry); errSendRetry != nil {
					if shouldFallbackCodexWebsocketSendErrorToHTTP(sess, conn, errSendRetry, upstreamBody) {
						e.clearUpstreamConn(sess, conn, "send_pre_payload_close_http_fallback", errSendRetry, false)
	...
	go func() {
		...
		for {
			...
			if errRead != nil {
				...
				if sess != nil && !receivedUpstreamPayload && !sentPayload && isCodexWebsocketStaleTerminalCloseError(errRead) {
					if !canRetryCodexWebsocketRequestAfterStaleTerminalClose(sess, upstreamBody) {
					...
					if errSendRetry := writeCodexWebsocketMessage(sess, conn, wsReqBodyRetry); errSendRetry != nil {
						...
						if shouldFallbackCodexWebsocketSendErrorToHTTP(sess, conn, errSendRetry, upstreamBody) {
							...
							e.clearUpstreamConn(sess, conn, terminateReason, errSendRetry, false)
							fallbackReq, fallbackOpts := codexHTTPFallbackRequest(req, opts)
				...
				if sess != nil && !receivedUpstreamPayload && !sentPayload && shouldFallbackCodexWebsocketPrePayloadReadErrorToHTTP(sess, conn, errRead, upstreamBody) {
				...
				if sess != nil && !receivedUpstreamPayload && !sentPayload && shouldRetryCodexWebsocketPrePayloadReadError(errRead, readRetryAttempt) && !codexWebsocketRequestCannotRetryFresh(sess, conn, upstreamBody) {
				...
				if !receivedUpstreamPayload && !sentPayload && isCodexWebsocketMessageTooBigError(errRead) && canFallbackCodexWebsocketRequestToHTTP(sess, conn, upstreamBody) {
			...
			if streamErr, terminalBody, ok := codexTerminalFailureErr(payload); ok && !hasStatuslessErr {
				terminateReason = "upstream_error"
...
func codexWebsocketStatuslessErrorStatus(payload []byte) int {
	...
	if code == "" {
		...
		if errorValue.Type == gjson.String {
			code = strings.TrimSpace(errorValue.String())
		}
	}
