...
// Connectivity test using Copilot provider with model "grok-code-fast-1" to ensure it routes and returns content.
func TestCopilot_GrokCodeFast1_Connectivity(t *testing.T) {
    // Fake upstream Copilot endpoint returning a minimal non-stream JSON completion
    mux := http.NewServeMux()
    mux.HandleFunc("/chat/completions", func(w http.ResponseWriter, r *http.Request) {
        w.Header().Set("Content-Type", "application/json")
    ...
    // Build an OpenAI-compatible chat request targeting grok-code-fast-1
    payload := []byte(`{"model":"grok-code-fast-1","messages":[{"role":"user","content":"hi"}]}`)
    req := cliproxyexecutor.Request{Model: "grok-code-fast-1", Payload: payload}
    opts := cliproxyexecutor.Options{SourceFormat: sdktranslator.FromString("openai"), OriginalRequest: payload}
