...
const (
	...
	defaultAntigravityAgent        = "antigravity/1.22.2 darwin/arm64" // fallback only; overridden at runtime by misc.AntigravityUserAgent()
...
func (e *AntigravityExecutor) refreshToken(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	// Use the same User-Agent as API calls to avoid fingerprint mismatch at the token endpoint.
	httpReq.Header.Set("User-Agent", resolveUserAgent(auth))
...
// the request payload so the API uses paid AI credits instead of free quota.
func antigravityInjectCreditTypes(payload []byte) []byte {
	result, _ := sjson.SetRawBytes(payload, "enabledCreditTypes", []byte(`["GOOGLE_ONE_AI"]`))
	return result
}
