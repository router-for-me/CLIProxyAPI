...
// It copies the shared proxy-aware client so Antigravity-specific transport tweaks
// never mutate the shared client cache.
func newAntigravityHTTPClient(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, timeout time.Duration) *http.Client {
	antigravityTransportOnce.Do(initAntigravityTransport)

	baseClient := helps.NewProxyAwareHTTPClient(ctx, cfg, auth, timeout)
	client := *baseClient
...
func (e *AntigravityExecutor) attemptCreditsFallback(
	...
	if !antigravityCreditsRetryEnabled(e.cfg) || !cliproxyauth.AntigravityCreditsRequested(ctx) {
		return nil, false
	}
...
func (e *AntigravityExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	translated, originalTranslated := helps.TranslateRequestWithOriginal(e.cfg, from, to, baseModel, req.Payload, originalPayload, false)
...
attemptLoop:
	for attempt := 0; attempt < attempts; attempt++ {
		...
		for idx, baseURL := range baseURLs {
			...
			if useCredits || antigravityShouldPreferCredits(auth, baseModel, time.Now()) {
