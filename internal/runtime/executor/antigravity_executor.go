...
func geminiToAntigravity(modelName string, payload []byte, projectID string) []byte {
	...
	if requestType == "web_search" {
		if modelInfo := registry.LookupModelInfo(resolvedModel, "antigravity"); modelInfo != nil && modelInfo.Thinking != nil {
			budgetResult := gjson.GetBytes(template, "request.generationConfig.thinkingConfig.thinkingBudget")
			if budgetResult.Exists() {
				budget := int(budgetResult.Int())
				support := modelInfo.Thinking
				if budget > 0 && support.Max > 0 && budget > support.Max {
					template, _ = sjson.SetBytes(template, "request.generationConfig.thinkingConfig.thinkingBudget", support.Max)
				} else if budget == 0 && !support.ZeroAllowed && support.Min > 0 {
					template, _ = sjson.SetBytes(template, "request.generationConfig.thinkingConfig.thinkingBudget", support.Min)
	...
	if projectID != "" {
		...
		template, _ = sjson.SetBytes(template, "project", fallbackProjectID)
