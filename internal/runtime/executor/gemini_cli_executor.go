...
func (e *GeminiCLIExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	basePayload, originalTranslated := helps.TranslateRequestWithOriginal(e.cfg, from, to, baseModel, req.Payload, originalPayload, false)
...
func (e *GeminiCLIExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	originalPayload := originalPayloadSource
	basePayload, originalTranslated := helps.TranslateRequestWithOriginal(e.cfg, from, to, baseModel, req.Payload, originalPayload, true)
	...
	for idx, attemptModel := range models {
		...
		go func(resp *http.Response, reqBody []byte, attemptModel string) {
			...
			if opts.Alt == "" {
				var param any
				errStream := helps.ReadStreamLines(resp.Body, func(line []byte) error {
...
func setJSONField(body []byte, key, value string) []byte {
	...
	updated, err := helps.SetJSONBytes(body, key, value)
	if err != nil {
		return body
	}
	return updated
...
func deleteJSONField(body []byte, key string) []byte {
	...
	updated, err := helps.DeleteJSONBytes(body, key)
	if err != nil {
		return body
	}
	return updated
