...
func ApplyPayloadConfigWithRootLazy(cfg *config.Config, model, protocol, root string, payload []byte, original func() []byte, requestedModel string) []byte {
	if cfg == nil || len(payload) == 0 {
		return payload
	}
	rules := cfg.Payload
	...
	for i := range rules.Default {
		...
		ruleSource := loadSource()
		for path, value := range rule.Params {
			fullPath := buildPayloadPath(root, path)
			if fullPath == "" {
				continue
			}
			if gjson.GetBytes(ruleSource, fullPath).Exists() {
				continue
			}
			if _, ok := appliedDefaults[fullPath]; ok {
				continue
