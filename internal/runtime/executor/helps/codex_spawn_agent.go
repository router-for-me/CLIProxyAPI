...
func OptimizeCodexMultiAgentV2Request(ctx context.Context, headers http.Header, payload []byte, cfg *config.Config) ([]byte, bool) {
	if cfg == nil || !cfg.Codex.OptimizeMultiAgentV2 || !isCodexMultiAgentClient(codexClientUserAgent(ctx, headers)) {
	...
	updated := rewriteCodexSpawnAgentTools(payload, toolPaths, models)
