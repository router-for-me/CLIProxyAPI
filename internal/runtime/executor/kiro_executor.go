	...
	for endpointIdx := 0; endpointIdx < len(endpointConfigs); endpointIdx++ {
		...
		for attempt := 0; attempt <= maxRetries; attempt++ {
			...
			if usageInfo.InputTokens == 0 &&
				usageInfo.CacheReadInputTokens == 0 &&
				usageInfo.CacheCreationInputTokens == 0 &&
...
func usageDetailForInternalStats(detail usage.Detail, estimatedCR, estimatedCW bool) usage.Detail {
	if !estimatedCR && !estimatedCW {
		return detail
	}
	var moved int64
	if estimatedCR {
		moved += detail.CacheReadInputTokens
		detail.CacheReadInputTokens = 0
	}
	if estimatedCW {
		moved += detail.CacheCreationInputTokens
		detail.CacheCreationInputTokens = 0
	}
	if moved == 0 {
		// Officially-reported sides only; nothing to strip.
		detail.CachedTokens = detail.CacheReadInputTokens + detail.CacheCreationInputTokens
		return detail
	}
	detail.InputTokens += moved
	detail.CachedTokens = detail.CacheReadInputTokens + detail.CacheCreationInputTokens
	return detail
}
...
func (e *KiroExecutor) parseEventStream(body io.Reader, model string) (string, []kiroclaude.KiroToolUse, usage.Detail, string, bool, bool, error) {
	...
	var hasOfficialCacheRead bool         // Whether tokenUsage supplied cacheReadInputTokens.
	var hasOfficialCacheWrite bool        // Whether tokenUsage supplied cacheWriteInputTokens.
	...
	for {
		...
		case "supplementaryWebLinksEvent":
			// inputTokens here is total input — skip when upstream already gave
			// uncached-only via tokenUsage.uncachedInputTokens.
			if inputTokens, ok := event["inputTokens"].(float64); ok && !hasUncachedInputTokens {
		...
		case "messageMetadataEvent", "metadataEvent":
			...
			// Skip InputTokens when uncached-only was already reported (incl. the
			// all-cached case where uncached=0); the legacy field is total input.
			if usageInfo.InputTokens == 0 && !hasUncachedInputTokens {
		...
		case "usageEvent", "usage":
			// Handle dedicated usage events. inputTokens here is the total input
			// (uncached + cached), so don't overwrite if upstream already gave us
			// the uncached-only value via tokenUsage.uncachedInputTokens.
			if inputTokens, ok := event["inputTokens"].(float64); ok && !hasUncachedInputTokens {
				usageInfo.InputTokens = int64(inputTokens)
		...
		case "metricsEvent":
			...
			if metrics, ok := event["metricsEvent"].(map[string]interface{}); ok {
				if inputTokens, ok := metrics["inputTokens"].(float64); ok && !hasUncachedInputTokens {
					usageInfo.InputTokens = int64(inputTokens)
				}
		...
		// Also check nested supplementaryWebLinksEvent. inputTokens here is the
		// total input — skip when upstream already gave uncached-only.
		if usageEvent, ok := event["supplementaryWebLinksEvent"].(map[string]interface{}); ok {
			if inputTokens, ok := usageEvent["inputTokens"].(float64); ok && !hasUncachedInputTokens {
				usageInfo.InputTokens = int64(inputTokens)
			}
...
func (e *KiroExecutor) streamToChannel(ctx context.Context, body io.Reader, out chan<- cliproxyexecutor.StreamChunk, targetFormat sdktranslator.Format, model string, originalReq, claudeBody []byte, reporter *usageReporter, thinkingEnabled bool) {
	...
	var hasOfficialCacheWrite bool        // Whether tokenUsage supplied cacheWriteInputTokens.
	var hasUncachedInputTokens bool       // Whether InputTokens explicitly means uncached input.
	var estimatedCacheRead bool           // Whether end-of-stream estimator filled in cache_read.
	var estimatedCacheWrite bool          // Whether end-of-stream estimator filled in cache_creation.
	...
	defer func() {
		reporter.publish(ctx, usageDetailForInternalStats(totalUsage, estimatedCacheRead, estimatedCacheWrite))
	}()
	...
	for {
		...
		default:
			...
			// Check for token counts in unknown events. inputTokens/prompt_tokens
			// here are total input — skip when upstream already gave uncached-only.
			if inputTokens, ok := event["inputTokens"].(float64); ok && !hasUncachedInputTokens {
		...
		case "messageMetadataEvent", "metadataEvent":
			...
			// Skip InputTokens when uncached-only was already reported (incl. the
			// all-cached case where uncached=0); the legacy field is total input.
			if totalUsage.InputTokens == 0 && !hasUncachedInputTokens {
				if inputTokens, ok := metadata["inputTokens"].(float64); ok {
					totalUsage.InputTokens = int64(inputTokens)
					hasUpstreamUsage = true
		...
		// Check nested usage event. inputTokens here is total input — skip
		// when upstream already gave uncached-only.
		if usageEvent, ok := event["supplementaryWebLinksEvent"].(map[string]interface{}); ok {
			if inputTokens, ok := usageEvent["inputTokens"].(float64); ok && !hasUncachedInputTokens {
				totalUsage.InputTokens = int64(inputTokens)
			}
