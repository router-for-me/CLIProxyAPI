...
func TestCodexWebsocketsExecuteStreamReconcilesClaudeUsage(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		created := []byte(`{"type":"response.created","response":{"id":"resp-1","model":"gpt-5.6-sol","usage":{"input_tokens":0}}}`)
	...
	start, ok := firstClaudeEventData(stream.String(), "message_start")
	if !ok {
	...
	delta, ok := firstClaudeEventData(stream.String(), "message_delta")
	if !ok {
