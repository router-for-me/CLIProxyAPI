...
func (e *BedrockExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	originalTranslated := sdktranslator.TranslateRequest(from, to, translateModel, originalPayloadSource, false)
	translated := sdktranslator.TranslateRequest(from, to, translateModel, payloadForTranslate, false)
...
func (e *BedrockExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	originalTranslated := sdktranslator.TranslateRequest(from, to, translateModel, originalPayloadSource, true)
	translated := sdktranslator.TranslateRequest(from, to, translateModel, payloadForTranslate, true)
	originalTranslated = stripBedrockUnsupportedToolConfig(originalTranslated, effectiveModelInfo)
	translated = stripBedrockUnsupportedToolConfig(translated, effectiveModelInfo)
...
func (e *BedrockExecutor) CountTokens(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	countTokensModel := resolveBedrockConfiguredModelName(e.cfg, auth, baseModel)
	if countTokensModel == "" {
		// Fallback: use baseModel if configured name not found
		countTokensModel = baseModel
	}
...
func parseBedrockUsage(body []byte) usage.Detail {
	...
	if usage := root.Get("usage"); usage.Exists() {
		...
		if cacheWrite := usage.Get("cacheWriteInputTokens"); cacheWrite.Exists() {
			detail.ReasoningTokens += cacheWrite.Int()
		}
...
func readBedrockEvent(r io.Reader) ([]byte, error) {
	...
	if exceptionType != "" {
		...
		if strings.EqualFold(strings.TrimSpace(exceptionType), "throttlingException") {
