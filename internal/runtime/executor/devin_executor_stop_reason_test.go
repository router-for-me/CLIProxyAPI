...
// output_item.done must carry status "completed" (AI SDK schema requirement) and
// truncation must surface as response.incomplete / max_output_tokens.
func TestStreamDevinFrames_OpenAIResponsesTruncatedToolCallHasStatusAndIncomplete(t *testing.T) {
	...
	var doneStatus, terminalType, incompleteReason string
	for _, ev := range events {
	...
	if doneStatus != "completed" {
		t.Fatalf("function_call output_item.done status = %q, want completed (events=%d)", doneStatus, len(events))
	}
