...
func TestShouldRetryCodexWebsocketSendError(t *testing.T) {
	if !shouldRetryCodexWebsocketSendError(websocket.ErrCloseSent, 0) {
		t.Fatal("close-sent send error should be retryable")
	}
	if shouldRetryCodexWebsocketSendError(websocket.ErrCloseSent, codexResponsesWebsocketSendRetryLimit) {
		t.Fatal("close-sent send error should stop retrying after the limit")
	}
	if shouldRetryCodexWebsocketSendError(fmt.Errorf("other error"), 0) {
		t.Fatal("non close-sent send error should not be retryable")
	}
}

func TestCodexWebsocketsExecuteStreamFallsBackToHTTPAfterCloseSentRetries(t *testing.T) {
	upgrader := websocket.Upgrader{CheckOrigin: func(*http.Request) bool { return true }}
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.EqualFold(r.Header.Get("Upgrade"), "websocket") {
			...
			case 2, 3, 4:
				closePayload := websocket.FormatCloseMessage(websocket.CloseNormalClosure, "retry closed")
				if errWrite := conn.WriteControl(websocket.CloseMessage, closePayload, time.Now().Add(time.Second)); errWrite != nil {
					errCh <- fmt.Errorf("write retry close %d: %w", connectionIndex, errWrite)
				}
		...
		_, _ = w.Write([]byte("data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp-http-close-sent\",\"output\":[],\"usage\":{\"input_tokens\":1,\"output_tokens\":1,\"total_tokens\":2}}}\n\n"))
	}))
	defer server.Close()
	...
	exec := NewCodexWebsocketsExecutor(&config.Config{SDKConfig: config.SDKConfig{DisableImageGeneration: config.DisableImageGenerationAll}})
	sessionID := "sess-close-sent-http-fallback"
	disconnectCh := exec.UpstreamDisconnectChan(sessionID)
	if disconnectCh == nil {
		t.Fatal("expected disconnect channel")
	}
	...
	req := cliproxyexecutor.Request{
		Model:   "gpt-5-codex",
		Payload: []byte(`{"type":"response.create","model":"gpt-5-codex","generate":false,"input":[{"type":"message","role":"user","content":"next"}]}`),
	}
	opts := cliproxyexecutor.Options{
		SourceFormat:   sdktranslator.FromString("openai-response"),
		ResponseFormat: sdktranslator.FromString("openai-response"),
	...
	assertCodexWebsocketCompletedChunk(t, result, "resp-http-close-sent")
	...
	if got := websocketConnections.Load(); got != 4 {
		t.Fatalf("websocket connection count = %d, want 4", got)
	}
	select {
	case errDisconnect, ok := <-disconnectCh:
		t.Fatalf("upstream disconnect signaled during close-sent HTTP fallback: err=%v ok=%v", errDisconnect, ok)
	case <-time.After(200 * time.Millisecond):
	}
	select {
	case errServer := <-errCh:
