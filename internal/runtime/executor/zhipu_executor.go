...
func (e *ZhipuExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	// If Claude Agent SDK for Python is explicitly disabled, return diagnostic error (no legacy fallback)
	if e.cfg != nil && !e.cfg.PythonAgent.Enabled {
		return cliproxyexecutor.Response{}, fmt.Errorf("python agent bridge disabled by config (claude-agent-sdk-for-python.enabled=false)")
	}
	...
	if bridgeURL == "" {
		...
		if err := validateLocalBridgeURL(bridgeURL); err != nil {
			return cliproxyexecutor.Response{}, err
		}
	}
...
func (e *ZhipuExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (<-chan cliproxyexecutor.StreamChunk, error) {
	// If python agent is explicitly disabled, return diagnostic error (no legacy fallback for streaming)
	if e.cfg != nil && !e.cfg.PythonAgent.Enabled {
		return nil, fmt.Errorf("python agent bridge disabled by config (claude-agent-sdk-for-python.enabled=false)")
	}
	token := zhipuCreds(auth)
	reporter := newUsageReporter(ctx, e.Identifier(), req.Model, auth)
	from := opts.SourceFormat
