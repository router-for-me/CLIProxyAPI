...
func applyClaudeHeadersWithNativeProfile(
	...
	policy, _ := resolveClaudeWirePolicy(cfg, auth, apiKey, confirmedClaudeCode)
	if !policy.Cloak && !policy.OAuth && !policy.ConfirmedClaudeCode {
		applyUncloakedClaudeHeaders(r, auth, incomingHeaders, extraBetas, stream)
		return nil
	}
...
// applyUncloakedClaudeHeaders keeps an API-key request protocol-correct without
// manufacturing a Claude Code identity. Caller-supplied client headers pass
// through, but no CLI user agent, device profile, session ID, request ID, or
// Stainless fingerprint is generated by the gateway. OAuth credentials retain
// their credential-bound identity because the upstream OAuth protocol requires it.
func applyUncloakedClaudeHeaders(r *http.Request, auth *cliproxyauth.Auth, incomingHeaders http.Header, extraBetas []string, stream bool) {
	shouldPass := func(name string) bool {
	...
	for name, values := range incomingHeaders {
		if !shouldPass(name) {
			continue
		}
	...
	betas := strings.TrimSpace(strings.Join(r.Header.Values("Anthropic-Beta"), ","))
	seenBetas := make(map[string]struct{})
	for _, beta := range strings.Split(betas, ",") {
		if beta = strings.TrimSpace(beta); beta != "" {
			seenBetas[beta] = struct{}{}
		}
	}
	for _, beta := range extraBetas {
		beta = strings.TrimSpace(beta)
		...
		if _, exists := seenBetas[beta]; exists {
			continue
		}
		if betas != "" {
			betas += ","
	...
	if _, supplied := r.Header["User-Agent"]; !supplied {
		// An explicitly empty value suppresses net/http's Go-http-client default.
