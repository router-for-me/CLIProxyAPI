...
func TestBuildConfigChangeDetails_RelaxedSystemPromptUsesEffectiveDefault(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "omitted field to disabled", old: &config.CloakConfig{}, new: &config.CloakConfig{RelaxedSystemPrompt: &disabled}},
		{name: "absent block to disabled", old: nil, new: &config.CloakConfig{RelaxedSystemPrompt: &disabled}},
		{name: "disabled to absent block", old: &config.CloakConfig{RelaxedSystemPrompt: &disabled}, new: nil},
		{name: "omitted field to enabled", old: &config.CloakConfig{}, new: &config.CloakConfig{RelaxedSystemPrompt: &enabled}, want: "claude[0].cloak.relaxed-system-prompt: false -> true"},
		{name: "absent block to enabled", old: nil, new: &config.CloakConfig{RelaxedSystemPrompt: &enabled}, want: "claude[0].cloak.relaxed-system-prompt: false -> true"},
		{name: "enabled to absent block", old: &config.CloakConfig{RelaxedSystemPrompt: &enabled}, new: nil, want: "claude[0].cloak.relaxed-system-prompt: true -> false"},
	}
