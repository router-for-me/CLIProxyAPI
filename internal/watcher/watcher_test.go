...
func TestBuildAPIKeyClientsCounts(t *testing.T) {
	cfg := &config.Config{
		...
		KimiKey: []config.KimiKey{
			{APIKey: "k1", Service: config.KimiServiceCodingPlan},
			{APIKey: "k2", Service: config.KimiServiceOpenPlatform, Region: config.KimiRegionDomestic},
		},
	...
	gemini, vertex, claude, codex, xai, kimi, compat := BuildAPIKeyClients(cfg)
	if gemini != 3 || vertex != 1 || claude != 1 || codex != 2 || xai != 1 || kimi != 2 || compat != 2 {
		t.Fatalf("unexpected counts: %d %d %d %d %d %d %d", gemini, vertex, claude, codex, xai, kimi, compat)
	}
}
