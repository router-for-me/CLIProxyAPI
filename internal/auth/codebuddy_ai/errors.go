...
var (
	ErrPollingTimeout   = errors.New("codebuddy-ai: polling timeout, user did not authorize in time")
	ErrAccessDenied     = errors.New("codebuddy-ai: access denied by user")
	ErrTokenFetchFailed = errors.New("codebuddy-ai: failed to fetch token from server")
	ErrJWTDecodeFailed  = errors.New("codebuddy-ai: failed to decode JWT token")
)
