...
const (
	...
	// refreshSkew is the time buffer before token expiry to trigger refresh
...
func (f *AntigravityFetcher) FetchQuota(ctx context.Context, auth *coreauth.Auth) (*ProviderQuotaData, error) {
	...
	if err != nil {
		log.Warnf("antigravity quota: failed to ensure access token: %v", err)
...
func (f *AntigravityFetcher) ensureAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	if expiry.After(time.Now().Add(refreshSkew)) {
		// Token is still valid
		return accessToken, nil
	}

	// Token is expired or about to expire, try to refresh
	log.Debugf("antigravity quota: access token expired or expiring soon, attempting refresh")

	refreshToken := f.extractRefreshToken(auth)
	if refreshToken == "" {
		// No refresh token, return existing access token (it may still work)
		log.Debugf("antigravity quota: no refresh token available, using existing access token")
		return accessToken, nil
	}
	...
	if err != nil {
		log.Warnf("antigravity quota: failed to refresh token: %v", err)
...
func (f *AntigravityFetcher) extractRefreshToken(auth *coreauth.Auth) string {
	if auth.Metadata == nil {
		return ""
	}
...
func (f *AntigravityFetcher) tokenExpiry(auth *coreauth.Auth) time.Time {
	if auth.Metadata == nil {
		return time.Time{}
	}
...
func (f *AntigravityFetcher) refreshAccessToken(ctx context.Context, auth *coreauth.Auth, refreshToken string) (string, error) {
	form := url.Values{}
	...
	log.Debugf("antigravity quota: token refreshed successfully, new expiry: %s", auth.Metadata["expired"])
