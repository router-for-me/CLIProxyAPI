...
func newPersistentUsageTestRouter(h *Handler) *gin.Engine {
	router := gin.New()
...
func TestGetUsageStatisticsReturnsSimplifiedShape(t *testing.T) {
	...
	router := newPersistentUsageTestRouter(&Handler{})
...
func TestGetUsageStatisticsReturnsEmptyWhenStoreUnavailable(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	if len(payload) != 0 {
		t.Fatalf("payload = %v, want empty usage", payload)
	}
}
...
func TestDeleteUsageRecordsRejectsUnavailableStore(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodDelete, "/v0/management/usage", strings.NewReader(`{"ids":["record-1"]}`))
	router.ServeHTTP(rec, req)

	if rec.Code != http.StatusBadRequest {
		t.Fatalf("status = %d, want %d body=%s", rec.Code, http.StatusBadRequest, rec.Body.String())
	}
	if !strings.Contains(rec.Body.String(), "\"error\":\"usage store unavailable\"") {
		t.Fatalf("body = %s, want usage store unavailable error", rec.Body.String())
	}
}
