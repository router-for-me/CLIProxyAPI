...
func (h *Handler) PutPackycode(c *gin.Context) {
	...
	if err := json.Unmarshal(data, &body); err != nil {
		...
		var wrapper struct {
			Packycode *config.PackycodeConfig `json:"packycode"`
		}
...
func (h *Handler) PatchPackycode(c *gin.Context) {
	...
	if v, ok := raw["defaults"]; ok {
		if m, ok2 := v.(map[string]any); ok2 {
			if _, ok3 := m["model"]; ok3 {
				cur.Defaults.Model = body.Defaults.Model
			}
			if _, ok3 := m["model-reasoning-effort"]; ok3 {
				cur.Defaults.ModelReasoningEffort = body.Defaults.ModelReasoningEffort
			}
		}
	}
	if v, ok := raw["credentials"]; ok {
		if m, ok2 := v.(map[string]any); ok2 {
			if _, ok3 := m["openai-api-key"]; ok3 {
				cur.Credentials.OpenAIAPIKey = body.Credentials.OpenAIAPIKey
			}
		}
	}
...
func normalizePackycode(pc *config.PackycodeConfig) {
	...
	if pc.Defaults.Model == "" {
		pc.Defaults.Model = "gpt-5"
	}
