...
func (h *Handler) PatchZhipuKey(c *gin.Context) {
	var body struct {
		Index *int             `json:"index"`
		Match *string          `json:"match"`
		Value *config.ZhipuKey `json:"value"`
	}
	if err := c.ShouldBindJSON(&body); err != nil || body.Value == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	}
	if body.Index != nil && *body.Index >= 0 && *body.Index < len(h.cfg.ZhipuKey) {
		h.cfg.ZhipuKey[*body.Index] = *body.Value
		h.persist(c)
		return
	}
	if body.Match != nil {
		for i := range h.cfg.ZhipuKey {
			if h.cfg.ZhipuKey[i].APIKey == *body.Match {
				h.cfg.ZhipuKey[i] = *body.Value
				h.persist(c)
				return
			}
		}
...
func (h *Handler) DeleteZhipuKey(c *gin.Context) {
	...
	if idxStr := c.Query("index"); idxStr != "" {
		var idx int
		if _, err := fmt.Sscanf(idxStr, "%d", &idx); err == nil && idx >= 0 && idx < len(h.cfg.ZhipuKey) {
			h.cfg.ZhipuKey = append(h.cfg.ZhipuKey[:idx], h.cfg.ZhipuKey[idx+1:]...)
			h.persist(c)
			return
		}
	}
