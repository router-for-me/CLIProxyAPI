...
// SetAPIKeyUsageService updates the persistent downstream key accounting service.
func (h *Handler) SetAPIKeyUsageService(service *apikeyusage.Service) {
	if h == nil {
		return
	}
	h.mu.Lock()
	h.apiKeyUsage = service
	h.mu.Unlock()
}
