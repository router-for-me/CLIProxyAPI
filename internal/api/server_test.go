...
func TestManagementControlPanelInvalidatesCacheWhenFileIdentityChanges(t *testing.T) {
	staticDir := t.TempDir()
	t.Setenv("MANAGEMENT_STATIC_PATH", staticDir)
	assetPath := filepath.Join(staticDir, "management.html")
	initial := []byte("<html><head></head><body>first</body></html>")
	replacement := []byte("<html><head></head><body>later</body></html>")
	if len(initial) != len(replacement) {
		t.Fatalf("test assets have different lengths: %d != %d", len(initial), len(replacement))
	}
	if errWrite := os.WriteFile(assetPath, initial, 0o600); errWrite != nil {
		t.Fatalf("write initial management asset: %v", errWrite)
	}
	initialInfo, errStat := os.Stat(assetPath)
	if errStat != nil {
		t.Fatalf("stat initial management asset: %v", errStat)
	}
	server := newTestServer(t)
	...
	if body := requestBody(); !strings.Contains(body, ">first<") {
		t.Fatalf("initial management asset was not served: %s", body)
	}

	replacementPath := filepath.Join(staticDir, "management.html.replacement")
	if errWrite := os.WriteFile(replacementPath, replacement, 0o600); errWrite != nil {
		t.Fatalf("write replacement management asset: %v", errWrite)
	}
	if errTimes := os.Chtimes(replacementPath, initialInfo.ModTime(), initialInfo.ModTime()); errTimes != nil {
		t.Fatalf("restore replacement management asset mtime: %v", errTimes)
	}
	if errRename := os.Rename(replacementPath, assetPath); errRename != nil {
		t.Fatalf("atomically replace management asset: %v", errRename)
	}
	replacementInfo, errStat := os.Stat(assetPath)
	if errStat != nil {
		t.Fatalf("stat replacement management asset: %v", errStat)
	}
	if replacementInfo.Size() != initialInfo.Size() || !replacementInfo.ModTime().Equal(initialInfo.ModTime()) {
		t.Fatalf(
			"replacement metadata changed: size=%d/%d mtime=%v/%v",
	...
	if os.SameFile(initialInfo, replacementInfo) {
		t.Fatal("atomic replacement did not change file identity")
	}

	body := requestBody()
	if !strings.Contains(body, ">later<") || strings.Contains(body, ">first<") {
		t.Fatalf("management asset cache returned stale replacement content: %s", body)
	}
	for _, scriptPath := range []string{managementasset.RequestLogUsageScriptPath, managementasset.LogQAScriptPath} {
		if strings.Count(body, scriptPath) != 1 {
			t.Fatalf("script %s count = %d, want 1 body=%s", scriptPath, strings.Count(body, scriptPath), body)
		}
	}
}
...
func TestCustomManagementRoutesRegistered(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "test-management-key")
	server := newTestServer(t)
	...
	want := []string{
		...
		"GET /v0/management/log-qa/sessions/logs",
		"GET /v0/management/log-qa/runs",
		"POST /v0/management/log-qa/run",
		"HEAD /management.html",
		"GET " + managementasset.RequestLogUsageScriptPath,
		"HEAD " + managementasset.RequestLogUsageScriptPath,
		"GET " + managementasset.LogQAScriptPath,
		"HEAD " + managementasset.LogQAScriptPath,
	}
...
func TestCustomManagementRoutesRequireManagementAuth(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "test-management-key")
	server := newTestServer(t)
	...
	authorized := httptest.NewRequest(http.MethodGet, "/v0/management/request-log-usage", nil)
	authorized.Header.Set("Authorization", "Bearer test-management-key")
	response := httptest.NewRecorder()
	server.engine.ServeHTTP(response, authorized)
	if response.Code == http.StatusUnauthorized {
		t.Fatalf("authorized route still returned 401: %s", response.Body.String())
	}
}

func TestManagementControlPanelInjectsBothCustomScriptsOnce(t *testing.T) {
	staticDir := t.TempDir()
	t.Setenv("MANAGEMENT_STATIC_PATH", staticDir)
	html := `<html><head><script type="module" src="official.js"></script></head><body></body></html>`
	if errWrite := os.WriteFile(filepath.Join(staticDir, "management.html"), []byte(html), 0o600); errWrite != nil {
		t.Fatalf("write management asset: %v", errWrite)
	}
	server := newTestServer(t)
	request := httptest.NewRequest(http.MethodGet, "/management.html", nil)
	...
	for _, scriptPath := range []string{managementasset.RequestLogUsageScriptPath, managementasset.LogQAScriptPath} {
		if strings.Count(body, scriptPath) != 1 {
			t.Fatalf("script %s count != 1: %s", scriptPath, body)
		}
		if strings.Index(body, scriptPath) > strings.Index(body, "official.js") {
			t.Fatalf("script %s was injected after official.js", scriptPath)
		}
	}
}

func TestLogQAScriptRoute(t *testing.T) {
	server := newTestServer(t)
	for _, method := range []string{http.MethodGet, http.MethodHead} {
		request := httptest.NewRequest(method, managementasset.LogQAScriptPath, nil)
		response := httptest.NewRecorder()
		server.engine.ServeHTTP(response, request)
		if response.Code != http.StatusOK {
		...
		if response.Header().Get("Cache-Control") != "no-store" ||
			...
			!strings.HasPrefix(response.Header().Get("Content-Type"), "application/javascript") {
...
func TestHomeEnabledHidesCustomManagementAssets(t *testing.T) {
	server := newTestServer(t)
	server.cfg.Home.Enabled = true
	for _, path := range []string{
		"/management.html",
		managementasset.RequestLogUsageScriptPath,
		managementasset.LogQAScriptPath,
	} {
