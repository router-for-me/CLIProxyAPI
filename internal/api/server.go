...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if authManager != nil {
		...
		authManager.SetRequestBudget(time.Duration(cfg.RequestBudgetSeconds) * time.Second)
	}
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/default-reasoning-on-ingress-by-format", s.mgmt.GetDefaultReasoningOnIngressByFormat)
		mgmt.PUT("/default-reasoning-on-ingress-by-format", s.mgmt.PutDefaultReasoningOnIngressByFormat)
		mgmt.PATCH("/default-reasoning-on-ingress-by-format", s.mgmt.PutDefaultReasoningOnIngressByFormat)
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if s.handlers != nil && s.handlers.AuthManager != nil {
		...
		s.handlers.AuthManager.SetRequestBudget(time.Duration(cfg.RequestBudgetSeconds) * time.Second)
	}
