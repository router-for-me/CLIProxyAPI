...
// the calling key's APIKeyPolicy when the key is model-restricted. When the
// server is running in Home mode it delegates to the Home-mode listing path
// (which has its own auth model) before reaching the policy filter.
