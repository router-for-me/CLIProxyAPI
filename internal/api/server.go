...
func (s *Server) codexAlphaSearch(c *gin.Context) {
	...
	if s.handlers.AuthManager.HomeEnabled() {
		selection, err = s.handlers.AuthManager.SelectHomeAuthByKind(ctx, "codex", strings.TrimSpace(routing.Model), auth.AuthKindOAuth, selectionOpts)
		if selection != nil {
			selected = selection.CloneAuth()
		}
	} else {
		selected, err = s.handlers.AuthManager.SelectAuthByKind(ctx, "codex", strings.TrimSpace(routing.Model), auth.AuthKindOAuth, selectionOpts)
	}
	if err != nil {
