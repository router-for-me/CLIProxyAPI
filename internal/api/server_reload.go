...
func (s *Server) UpdateClientsContext(ctx context.Context, cfg *config.Config) bool {
	...
	if s.requestLogPolicy == nil {
		log.Error("request log policy unavailable; rejecting configuration reload")
		return false
	}
	policyReload := s.requestLogPolicy.BeginReload(cfg.APIKeysNoLog)
	policyReloadCommitted := false
	defer func() {
		if !policyReloadCommitted {
			policyReload.FailClosed()
		}
	}()
	...
	if !policyReload.Commit() {
		log.Error("request log policy reload was superseded")
		return false
	}
	policyReloadCommitted = true
	applyAPIKeyLoggingPolicies(s.requestLogger, cfg)
	return true
}
