...
func defaultRequestLoggerFactory(cfg *config.Config, configPath string) logging.RequestLogger {
	...
	applyAPIKeyLoggingPolicies(logger, cfg)
	logger.SetHomeEnabled(cfg != nil && cfg.Home.Enabled)
	return logger
}

func applyAPIKeyLoggingPolicies(target logging.RequestLogger, cfg *config.Config) {
	if target == nil || cfg == nil {
		return
	}
	if setter, ok := target.(interface{ SetAPIKeyNames([]string, []string) }); ok {
		setter.SetAPIKeyNames(cfg.APIKeys, cfg.APIKeyNames)
	}
	if setter, ok := target.(interface{ SetNoLogAPIKeys([]string) }); ok {
		setter.SetNoLogAPIKeys(cfg.APIKeysNoLog)
	}
}
