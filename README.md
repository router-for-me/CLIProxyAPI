...
- **Docker plugins across upgrades:** installed plugins live under `plugins.dir` (default `plugins/`). `docker-compose.yml` mounts `./plugins` → `/CLIProxyAPI/plugins` so **later** container recreations keep plugins. **First time you add the volume**, copy plugins out of the old container before recreate (bind mounts start empty and do not migrate existing image-layer files): `mkdir -p ./plugins && docker cp cli-proxy-api:/CLIProxyAPI/plugins/. ./plugins/`. Without a volume, plugins disappear when the container is recreated and must be reinstalled.
