...
- Gemini ネイティブのクライアントやブリッジでは、`/api/provider/google/v1beta/models/{model}:generateContent` や `...:streamGenerateContent` のような Google 形式の provider ルートを優先すると、リクエスト/レスポンス本文をネイティブ Gemini 形式のまま維持しやすくなります。
- OpenAI Responses の `developer` メッセージは、Gemini で未対応の role として転送するのではなく、Gemini の `systemInstruction` に折りたたまれます。
- CLIProxyAPI は Gemini API / Vertex へ転送する前に、中継専用の `includeServerSideToolInvocations` フラグを取り除き、混在したプロキシ経路で Google/Vertex から `400` が返るケースを減らします。
