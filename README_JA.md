...
セルフホスト型 CLIProxyAPI 向けのクラウドバックエンド使用量分析ダッシュボードです。Redis 互換の使用量キューを消費し、機密情報を除去して、スリム化したイベントと日次ロールアップを Supabase/Postgres に保存します。ローカルのフットプリントをほぼ増やさずに、アカウント/API key 別の使用量、Tokenトレンド、コレクターの状態、LiteLLM ベースのコスト推定を表示します。
