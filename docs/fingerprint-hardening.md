...
| 3 | Anthropic HTTP | **HTTP/1.1 request header order** matches the real client (live-captured claude-cli 2.1.153): a **case-sensitive ASCII sort** of the application headers, followed by the transport trailer `Connection`, `Host`, `Accept-Encoding`, `Content-Length` — not undici insertion order and not Go's alphabetical sort — over a keep-alive pooled uTLS connection. Wire casing is applied per `headerWireCasing` (lowercase `anthropic-*`/`x-app`/`x-api-key`, `X-Stainless-OS`, etc.). | (same as #2) |
| 4 | Anthropic Accept-Encoding | Non-stream requests advertise the live-captured default **`gzip, deflate, br, zstd`** (set + order, per real claude-cli 2.1.153) rather than a Go/ad-hoc value; zstd responses are still decoded. Streams use `identity`. | — |
