...
1. 缺失时从 example 生成  
   ...
   - `log-qa.yaml`  
   - `.env`  
2. 创建 `logs/`、`auths/`、`plugins/`、`data/`（插件二进制、数据库及运行状态需持久化）
3. 启动三个服务  
...
- compose 已挂载 `./data` → `/CLIProxyAPI/data`，用于持久化插件数据库；例如 CAP Token Usage Tracker 默认使用 `/CLIProxyAPI/data/token-usage-tracker.db`
- 若数据库已写在旧容器层中，首次使用新挂载前必须先停止服务，并将数据库复制到宿主机 `./data/`；直接重建会让新挂载遮住旧文件
