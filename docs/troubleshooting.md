...
| Orchids proxy path returns `404` or `model_not_found` | Orchids endpoint not wired as OpenAI-compatible provider entry | Verify `openai-compatibility` entry (`name`, `base-url`, `prefix`) and check `/v1/models` | Configure Orchids under `openai-compatibility` with `/v1` base URL and route as `orchids/<model>` |
