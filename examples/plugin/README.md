...
- `codex-tool-search-shim/`: Responses plugin that keeps the first request of a non-`codex` route from inlining the whole deferred tool catalog, by converting Codex client-executed tool search into ordinary function calling and promoting only tools returned by `tool_search_output`; it can also strict-complete `tool_search` schemas, inline local `$ref`s, and drop unsupported `custom` tools for native Responses models listed in `strict_responses_models`. See `codex-tool-search-shim/README.md`.
