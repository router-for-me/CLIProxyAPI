...
func (m *memStore) List(ctx context.Context) ([]*coreauth.Auth, error) {
	return append([]*coreauth.Auth(nil), m.saved...), nil
}
...
func TestManager_Copilot_HTTP_End2End(t *testing.T) {
	...
	exec := executor.NewCopilotExecutor(cfg)
	...
	if _, ok := got.Metadata["refresh_in"]; !ok {
		t.Fatalf("expected refresh_in present")
	}
	if _, ok := got.Metadata["expires_at"]; !ok {
		t.Fatalf("expected expires_at present")
	}
	...
	if persisted == nil {
		t.Fatalf("expected persisted record for %s", a.ID)
	}
