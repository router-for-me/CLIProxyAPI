...
type QuotaState struct {
	...
	// WindowResetAt records the next quota window reset time from upstream rate limit headers.
	WindowResetAt time.Time `json:"window_reset_at,omitempty"`
	// TokensRemaining tracks the remaining token budget reported by upstream headers.
	TokensRemaining int64 `json:"tokens_remaining,omitempty"`
	// TokensLimit tracks the total token limit reported by upstream headers.
