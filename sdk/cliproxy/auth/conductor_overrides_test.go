...
func TestManager_ShouldRetryAfterError_RespectsAuthRequestRetryOverride(t *testing.T) {
	...
	wait, shouldRetry := m.shouldRetryAfterError(context.Background(), &Error{HTTPStatus: 500, Message: "boom"}, 0, []string{"claude"}, model, maxWait)
	if shouldRetry {
	...
	_, shouldRetry = m.shouldRetryAfterError(context.Background(), &Error{HTTPStatus: 500, Message: "boom"}, 1, []string{"claude"}, model, maxWait)
	if shouldRetry {
