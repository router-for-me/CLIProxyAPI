...
type Result struct {
	...
	Error *Error
	// RateLimit holds parsed upstream rate limit data for quota-aware routing.
	RateLimit *RateLimitHint
}

// RateLimitHint carries parsed upstream rate limit header values.
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		result := Result{AuthID: auth.ID, Provider: provider, Model: routeModel, Success: errExec == nil, RateLimit: parseRateLimitHint(resp.Headers)}
		if errExec != nil {
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		go func(streamCtx context.Context, streamAuth *Auth, streamProvider string, streamChunks <-chan cliproxyexecutor.StreamChunk, hint *RateLimitHint) {
			defer close(out)
			var failed bool
			forward := true
			...
			if !failed {
				m.MarkResult(streamCtx, Result{AuthID: streamAuth.ID, Provider: streamProvider, Model: routeModel, Success: true, RateLimit: hint})
			}
		}(execCtx, auth.Clone(), provider, streamResult.Chunks, parseRateLimitHint(streamResult.Headers))
...
// Rate limit header constants for upstream quota tracking.
const (
...
// parseRateLimitHint extracts rate limit data from upstream response headers.
...
// applyRateLimitHint updates the auth's QuotaState from parsed rate limit data.
func applyRateLimitHint(auth *Auth, hint *RateLimitHint) {
	if hint == nil {
		return
	}
	if !hint.WindowResetAt.IsZero() {
		auth.Quota.WindowResetAt = hint.WindowResetAt
	}
	if hint.TokensRemaining > 0 {
		auth.Quota.TokensRemaining = hint.TokensRemaining
	}
	if hint.TokensLimit > 0 {
		auth.Quota.TokensLimit = hint.TokensLimit
	}
}
