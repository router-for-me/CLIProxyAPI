...
// Selector determines which strategy the authScheduler uses for credential
// rotation. Supported implementations: *RoundRobinSelector, *FillFirstSelector,
// *StickySelector. The scheduler handles all credential selection internally;
// Selector.Pick is no longer called at runtime (retained for interface compatibility).
...
// resolveAuth looks up an auth by ID and returns its clone, executor, and provider key.
// Used by sticky routing to return a cached auth that has been verified as ready.
func (m *Manager) resolveAuth(authID string) (*Auth, ProviderExecutor, string, error) {
	m.mu.RLock()
	auth, exists := m.auths[authID]
	if !exists || auth == nil || auth.Disabled {
		m.mu.RUnlock()
		return nil, nil, "", &Error{Code: "auth_not_found", Message: "sticky auth not found"}
	}
	...
	if !auth.indexAssigned {
		m.mu.RUnlock()
		m.mu.Lock()
		if current := m.auths[authID]; current != nil && !current.indexAssigned {
			current.EnsureIndex()
...
func (m *Manager) pickNext(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	// Sticky preference: reuse cached auth if it is ready (not in cooldown),
	// not already tried, and not overridden by an explicit pin.
	if sticky, ok := m.selector.(*StickySelector); ok && pinnedAuthIDFromMetadata(opts.Metadata) == "" {
		if preferred := sticky.LookupCachedAuth(opts.OriginalRequest, model); preferred != "" {
			if _, alreadyTried := tried[preferred]; !alreadyTried {
				if m.scheduler.isAuthReadyForModel(preferred, []string{provider}, model) {
					if auth, exec, _, err := m.resolveAuth(preferred); err == nil {
						return auth, exec, nil
					}
				}
			}
			// Cached auth not ready, already tried, or resolve failed — evict.
			sticky.EvictForPayload(opts.OriginalRequest, model)
			log.Debugf("sticky cache evicted for model %s in pickNext (not ready or tried)", model)
		}
	}
	...
	if sticky, ok := m.selector.(*StickySelector); ok {
		sticky.RecordBinding(opts.OriginalRequest, model, authCopy.ID)
	}
...
func (m *Manager) pickNextMixed(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	if sticky, ok := m.selector.(*StickySelector); ok && pinnedAuthIDFromMetadata(opts.Metadata) == "" {
		if preferred := sticky.LookupCachedAuth(opts.OriginalRequest, model); preferred != "" {
			if _, alreadyTried := tried[preferred]; !alreadyTried {
				if m.scheduler.isAuthReadyForModel(preferred, eligibleProviders, model) {
					if auth, executor, pkey, err := m.resolveAuth(preferred); err == nil {
						return auth, executor, pkey, nil
					}
				}
			}
			// Cached auth not ready, already tried, or resolve failed — evict.
			sticky.EvictForPayload(opts.OriginalRequest, model)
			log.Debugf("sticky cache evicted for model %s in pickNextMixed (not ready or tried)", model)
		}
	}
	...
	// Record sticky binding so the next request with the same session reuses this auth.
	if sticky, ok := m.selector.(*StickySelector); ok {
		sticky.RecordBinding(opts.OriginalRequest, model, authCopy.ID)
	}
	return authCopy, executor, providerKey, nil
