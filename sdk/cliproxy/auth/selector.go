...
// EarliestResetSelector prefers credentials whose upstream quota window resets
// soonest.  This implements the "eat perishable food first" pattern: the auth
// whose remaining tokens will expire (reset) earliest is used first, avoiding
// waste when multiple auths have independent reset schedules.
//
// When no rate limit data is available yet (cold start), behaviour degrades
// gracefully to fill-first ordering (sorted by ID).
type EarliestResetSelector struct{}

// Pick returns the auth whose WindowResetAt is earliest.  Auths without rate
// limit data are placed after those with data so that populated entries are
// preferred.
func (s *EarliestResetSelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	sort.SliceStable(available, func(i, j int) bool {
		ri := available[i].Quota.WindowResetAt
		rj := available[j].Quota.WindowResetAt
		iZero := ri.IsZero()
		jZero := rj.IsZero()
		// Auths with data sort before auths without data.
		if iZero != jZero {
			return !iZero
		}
		if !iZero && !jZero {
		...
		// Both zero: fall back to ID order (same as fill-first).
