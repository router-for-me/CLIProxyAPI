...
func TestApplyAuthFailureStateQuotaBackoffOncePerWindow(t *testing.T) {
	...
	applyAuthFailureState(auth, quotaErr, nil, now, false, defaultMaxTrustedQuotaCooldown)
	...
	applyAuthFailureState(auth, quotaErr, nil, now.Add(100*time.Millisecond), false, defaultMaxTrustedQuotaCooldown)
	if auth.Quota.BackoffLevel != 1 {
	...
	applyAuthFailureState(auth, quotaErr, nil, now.Add(2*time.Second), false, defaultMaxTrustedQuotaCooldown)
	...
	applyAuthFailureState(auth, quotaErr, &retryAfter, now.Add(3*time.Second), false, defaultMaxTrustedQuotaCooldown)
	if auth.Quota.BackoffLevel != 2 {
