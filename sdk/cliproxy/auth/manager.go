...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
    ...
    // Notify intelligent selector of result
    if intelligentSelector, ok := m.selector.(*IntelligentSelector); ok && intelligentSelector != nil {
        if result.Success {
            // For successful requests, we can estimate token count from response
            // or use a default value if not available
            intelligentSelector.RecordAuthSuccess(result.AuthID, result.Model, 0, 0)
        } else {
            intelligentSelector.RecordAuthError(result.AuthID, result.Model, result.Error)
        }
    }
