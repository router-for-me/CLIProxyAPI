...
func TestClampQuotaCooldown(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "166 hour reset is clamped", next: now.Add(166*time.Hour + 27*time.Minute), want: ceiling},
...
// TestMarkResult_AntigravityModelQuotaClampsAndDoesNotBlockSiblingModel verifies Issue #5843:
// When an Antigravity request (e.g. gemini-3.8-flash(high)) fails with a 166-hour weekly reset delay:
// 1. Its cooldown deadline must be clamped to maxQuotaCooldownCeiling.
// 2. Its canonical base model (gemini-3.8-flash) receives the clamped cooldown.
// 3. Sibling models (e.g. gemini-3-flash) remain schedulable and are NOT blocked by the aggregated quota.
func TestMarkResult_AntigravityModelQuotaClampsAndDoesNotBlockSiblingModel(t *testing.T) {
	previous := quotaCooldownDisabled.Load()
	quotaCooldownDisabled.Store(false)
	t.Cleanup(func() { quotaCooldownDisabled.Store(previous) })
	...
	const (
		...
		thinkingModel = "gemini-3.8-flash(high)"
		baseModel     = "gemini-3.8-flash"
		siblingModel  = "gemini-3-flash"
	)

	manager := NewManager(nil, &RoundRobinSelector{}, nil)
	auth := &Auth{
		ID:       "antigravity-auth-ceiling",
		Provider: provider,
		Status:   StatusActive,
	}
	...
	now := time.Now()
	weeklyResetDelay := 166*time.Hour + 27*time.Minute + 18*time.Second

	// MarkResult with gemini-3.8-flash(high) and 166h reset delay
	manager.MarkResult(context.Background(), Result{
		AuthID:          auth.ID,
		Provider:        provider,
		Model:           thinkingModel,
		Success:         false,
		RetryAfter:      &weeklyResetDelay,
		CredentialScope: false,
		Error:           &Error{HTTPStatus: http.StatusTooManyRequests, Message: "Individual quota reached. Resets in 166h27m18s."},
	})
	...
	// 1. Verify model state on canonical model is clamped to maxQuotaCooldownCeiling
	baseState := existingModelState(updated, canonicalModelKey(thinkingModel))
	if baseState == nil {
		t.Fatalf("expected model state for %q to exist", baseModel)
	}
	if !baseState.Quota.Exceeded {
		t.Fatalf("expected baseModel %q Quota.Exceeded = true", baseModel)
	}
	if baseState.Quota.NextRecoverAt.After(ceiling) {
		t.Fatalf("baseModel Quota.NextRecoverAt = %v, want clamped to <= %v", baseState.Quota.NextRecoverAt, ceiling)
	}

	// 2. Base model must be cooling down
	blockedBase, reasonBase, nextBase := isAuthBlockedForModel(updated, baseModel, now)
	if !blockedBase || reasonBase != blockReasonCooldown {
		t.Fatalf("isAuthBlockedForModel(%q) = (%v, %v), want (true, blockReasonCooldown)", baseModel, blockedBase, reasonBase)
	}
	if nextBase.After(ceiling) {
		t.Fatalf("base model blocked next = %v, want clamped to <= %v", nextBase, ceiling)
	}

	// 3. Crucial check: Sibling model (gemini-3-flash) must NOT be blocked!
	blockedSibling, reasonSibling, _ := isAuthBlockedForModel(updated, siblingModel, now)
	if blockedSibling {
		t.Fatalf("sibling model %q was blocked (reason=%v), want NOT blocked by single-model quota", siblingModel, reasonSibling)
	}
}

// TestMarkResult_AntigravityClaudeQuotaExhaustedDoesNotBlockGeminiModels verifies:
// When an Antigravity request for a Claude model (e.g. claude-opus-4-6-thinking) encounters
// an upstream 429 with 166h reset delay:
// 1. Claude model state is clamped to maxQuotaCooldownCeiling.
// 2. Gemini models (e.g. gemini-3.8-flash) are NOT blocked and remain immediately available.
func TestMarkResult_AntigravityClaudeQuotaExhaustedDoesNotBlockGeminiModels(t *testing.T) {
	previous := quotaCooldownDisabled.Load()
	quotaCooldownDisabled.Store(false)
	t.Cleanup(func() { quotaCooldownDisabled.Store(previous) })
	...
	auth := &Auth{
		ID:       "antigravity-auth-claude-quota",
		Provider: provider,
		Status:   StatusActive,
	}
	...
	now := time.Now()
	weeklyResetDelay := 166*time.Hour + 8*time.Minute + 44*time.Second

	// Claude Opus 429 on Antigravity
	manager.MarkResult(context.Background(), Result{
		AuthID:          auth.ID,
		Provider:        provider,
		Model:           claudeModel,
		...
		RetryAfter:      &weeklyResetDelay,
		CredentialScope: false,
		Error:           &Error{HTTPStatus: http.StatusTooManyRequests, Message: "Individual quota reached. Resets in 166h8m44s."},
	})
	...
	// 1. Claude model is blocked (clamped to maxQuotaCooldownCeiling)
	blockedClaude, reasonClaude, nextClaude := isAuthBlockedForModel(updated, claudeModel, now)
	if !blockedClaude || reasonClaude != blockReasonCooldown {
		t.Fatalf("isAuthBlockedForModel(%q) = (%v, %v), want (true, blockReasonCooldown)", claudeModel, blockedClaude, reasonClaude)
	}
	ceiling := now.Add(maxQuotaCooldownCeiling).Add(time.Minute)
	if nextClaude.After(ceiling) {
		t.Fatalf("claude model blocked next = %v, want clamped to <= %v", nextClaude, ceiling)
	}

	// 2. Crucial test: Gemini 3.8 Flash MUST NOT BE BLOCKED!
	blockedGemini, reasonGemini, _ := isAuthBlockedForModel(updated, geminiModel, now)
	if blockedGemini {
		t.Fatalf("gemini model %q was blocked (reason=%v), want NOT blocked when Claude hits quota limit", geminiModel, reasonGemini)
	}

	// 3. Selection of geminiModel on this auth succeeds
	selected, errSelect := manager.SelectAuth(context.Background(), provider, geminiModel, cliproxyexecutor.Options{})
	if errSelect != nil {
		t.Fatalf("SelectAuth(%q) error = %v, want success", geminiModel, errSelect)
	}
	if selected == nil || selected.ID != auth.ID {
		t.Fatalf("SelectAuth(%q) selected = %v, want %s", geminiModel, selected, auth.ID)
	}
}
