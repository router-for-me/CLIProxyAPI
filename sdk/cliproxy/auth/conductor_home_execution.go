...
func (m *Manager) executeHome(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, countTokens bool) (cliproxyexecutor.Response, error) {
	...
	for homeAuthCount := 1; ; homeAuthCount++ {
		...
		for _, upstreamModel := range models {
			...
			if countTokens {
				operation = cliproxyexecutor.RequestOperationCountTokens
			}
			execReq, execOpts, errIntercept = applyRequestAfterAuthInterceptor(execCtx, selection.Executor, selection.Provider, operation, execReq, execOpts, requestedModelAliasFromOptions(execOpts, routeModel))
			if errIntercept != nil {
				releaseAttempt()
