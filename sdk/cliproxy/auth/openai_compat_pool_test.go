...
func TestManagerExecute_OpenAICompatMiniMaxM3HighspeedRouteUsesStandardM3(t *testing.T) {
	...
	resp, err := m.Execute(context.Background(), []string{"pool"}, cliproxyexecutor.Request{
		Model:   model,
...
func TestManagerExecute_OpenAICompatLargeToolHistorySkipsMiniMaxM3Highspeed(t *testing.T) {
	...
	m := newOpenAICompatPoolTestManager(t, alias, []internalconfig.OpenAICompatibilityModel{
		...
		{Name: "mimo-v2.5-pro", Alias: alias},
	}, executor)
	...
	if string(resp.Payload) != "mimo-v2.5-pro" {
		t.Fatalf("payload = %q, want mimo-v2.5-pro", string(resp.Payload))
	}
	got := executor.ExecuteModels()
	if len(got) != 1 || got[0] != "mimo-v2.5-pro" {
		t.Fatalf("execute models = %v, want only mimo-v2.5-pro", got)
	}
}
