...
func TestExtractSessionInfoAllClients(t *testing.T) {
	...
	// 4b. OpenCode gateway native session header with a parent hierarchy header.
	// The affinity parent header names a parent bound as affinity:<id>, so the child
	// must reference that same namespace or the selector misses the parent binding.
	opencodeNativeHeaders := http.Header{
		"X-Opencode-Session":        []string{"oc-native-222"},
		"X-Parent-Session-Affinity": []string{"oc-native-parent-111"},
	}
	info, ok = ExtractSessionInfo(opencodeNativeHeaders, nil, nil)
	if !ok || info.ClientType != "opencode" || info.SessionID != "opencode:oc-native-222" || info.ParentSessionID != "affinity:oc-native-parent-111" || info.AgentName != "subagent" {
		t.Errorf("OpenCode native header mismatch: %+v", info)
	}

	// 4c. A generic parent header keeps the child's own namespace.
	opencodeGenericParentHeaders := http.Header{
		"X-Opencode-Session":  []string{"oc-native-333"},
		"X-Parent-Session-ID": []string{"oc-native-parent-222"},
	}
	info, ok = ExtractSessionInfo(opencodeGenericParentHeaders, nil, nil)
	if !ok || info.SessionID != "opencode:oc-native-333" || info.ParentSessionID != "opencode:oc-native-parent-222" {
		t.Errorf("OpenCode generic parent header mismatch: %+v", info)
	}

	// 4d. Body-only fork in thread_id
