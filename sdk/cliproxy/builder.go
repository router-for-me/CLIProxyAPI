...
func (b *Builder) Build() (*Service, error) {
    ...
    if coreManager == nil {
        ...
        // Create selector based on configuration
        selector := coreauth.NewSelectorFromConfig(b.cfg.IntelligentRouting.Enabled)
        coreManager = coreauth.NewManager(tokenStore, selector, nil)
        
        // Start cleanup routine if intelligent routing is enabled
        if b.cfg.IntelligentRouting.Enabled {
            if intelligentSelector, ok := selector.(*coreauth.IntelligentSelector); ok {
                retentionHours := b.cfg.IntelligentRouting.StatsRetentionHours
                if retentionHours <= 0 {
                    retentionHours = 24
                }
                cleanupMinutes := b.cfg.IntelligentRouting.CleanupIntervalMinutes
                if cleanupMinutes <= 0 {
                    cleanupMinutes = 60
                }
                coreauth.StartIntelligentSelectorCleanup(intelligentSelector, retentionHours, cleanupMinutes)
            }
        }
    }
