...
func isClaudeCompactRequest(rawJSON []byte) bool {
	...
	if text == "" || !strings.Contains(text, "your task is to create a detailed summary") {
		return false
	}
...
func stripClaudeCompactionCapsule(text string) (string, *claudeCompactionCapsule, bool, error) {
	start := strings.Index(text, claudeCompactionCapsulePrefix)
	if start < 0 {
	...
	end := encodedStart + relativeEnd
	if strings.Contains(text[end+len(claudeCompactionCapsuleSuffix):], claudeCompactionCapsulePrefix) {
		return "", nil, false, fmt.Errorf("multiple compaction capsules are not supported")
	}
	capsule, errDecode := decodeClaudeCompactionCapsule(text[encodedStart:end])
	if errDecode != nil {
		return "", nil, false, errDecode
	}
	remaining := strings.TrimSpace(text[:start] + text[end+len(claudeCompactionCapsuleSuffix):])
	return remaining, capsule, true, nil
}
...
func buildClaudeCompactSSE(clientResponse []byte, marker string) []byte {
	...
	appendClaudeSSEEvent(&out, "message_delta", map[string]any{
		...
		"usage": map[string]any{"output_tokens": response.Get("usage.output_tokens").Int()},
	})
