...
type ErrorDetail struct {
    ...
    // ResetTime conveys how long remains before the request can be retried.
    ResetTime string `json:"reset_time,omitempty"`

    // ResetTimestamp indicates the absolute time when the credential should recover.
    ResetTimestamp string `json:"reset_timestamp,omitempty"`
}
...
func (h *BaseAPIHandler) ExecuteStreamWithAuthManager(ctx context.Context, handlerType, modelName string, rawJSON []byte, alt string) (<-chan []byte, <-chan *interfaces.ErrorMessage) {
    ...
    if err != nil {
        ...
        errChan <- &interfaces.ErrorMessage{StatusCode: status, Error: err}
        close(errChan)
        return nil, errChan
    }
    ...
    go func() {
        ...
        for chunk := range chunks {
            if chunk.Err != nil {
                ...
                errChan <- &interfaces.ErrorMessage{StatusCode: status, Error: chunk.Err}
                return
            }
...
func firstString(details map[string]any, keys ...string) string {
    ...
    for _, key := range keys {
        ...
        default:
            if formatted := fmt.Sprintf("%v", v); formatted != "" && formatted != "<nil>" {
                return formatted
            }
        }
    }
...
func (h *BaseAPIHandler) WriteErrorResponse(c *gin.Context, msg *interfaces.ErrorMessage) {
    ...
    if coreErr, ok := msg.Error.(*coreauth.Error); ok {
        detail := ErrorDetail{
        ...
        if len(coreErr.Details) > 0 {
            if resetTime := firstString(coreErr.Details, "reset_time"); resetTime != "" {
                detail.ResetTime = resetTime
            }
            if resetTimestamp := firstString(coreErr.Details, "reset_timestamp", "reset_at"); resetTimestamp != "" {
                detail.ResetTimestamp = resetTimestamp
            }
        }
