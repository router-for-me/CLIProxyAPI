...
func TestCLISubFallbackChainCatalogIntersectionNotHardcodedOpus(t *testing.T) {
	reg := registry.GetGlobalRegistry()
	cases := []struct {
		primaryID, primaryOwner, cursorID, droidID, requested string
	}{
		{"test-cli-sub-claude-p", "anthropic", "cursor-claude-opus-5-high", "droid-opus-5", "claude-opus-5"},
		{"test-cli-sub-gpt-p", "openai", "cursor-gpt-5.6-sol-high", "droid-gpt-5.6-sol", "gpt-5.6-sol"},
		{"test-cli-sub-grok-p", "xai", "cursor-grok-4", "droid-grok-4", "grok-4"},
		{"test-cli-sub-sonnet-p", "anthropic", "cursor-claude-sonnet-5-high", "droid-sonnet-5", "claude-sonnet-5"},
	}
	for _, tc := range cases {
		t.Run(tc.requested, func(t *testing.T) {
			cursorClient := tc.primaryID + "-cursor"
			droidClient := tc.primaryID + "-droid"
			t.Cleanup(func() {
				reg.UnregisterClient(tc.primaryID)
				reg.UnregisterClient(cursorClient)
				reg.UnregisterClient(droidClient)
			})
			reg.RegisterClient(tc.primaryID, tc.primaryOwner, []*registry.ModelInfo{{ID: tc.requested, OwnedBy: tc.primaryOwner}})
			reg.RegisterClient(cursorClient, "cursor", []*registry.ModelInfo{{ID: tc.cursorID, OwnedBy: cliSubscriptionsOwner}})
			reg.RegisterClient(droidClient, "droid", []*registry.ModelInfo{{ID: tc.droidID, OwnedBy: cliSubscriptionsOwner}})

			chain := buildCLISubFallbackChain(reg, tc.requested)
			if len(chain) != 2 {
				t.Fatalf("fallback chain length = %d, want 2: %#v", len(chain), chain)
			}
			if chain[0].Model != tc.cursorID || chain[1].Model != tc.droidID {
				t.Fatalf("fallback order = %#v, want %s then %s", chain, tc.cursorID, tc.droidID)
			}
			if chain[0].PrimaryOwner != tc.primaryOwner {
				t.Fatalf("primary owner = %q, want %q", chain[0].PrimaryOwner, tc.primaryOwner)
			}
		})
	}
}
