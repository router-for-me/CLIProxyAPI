...
func (a GitHubCopilotAuthenticator) Login(ctx context.Context, cfg *config.Config, opts *LoginOptions) (*coreauth.Auth, error) {
	...
	fmt.Println("Starting Copilot authentication...")
	...
	if err != nil {
		...
		return nil, fmt.Errorf("copilot: %s", errMsg)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("copilot: failed to verify Copilot access - you may not have an active Copilot subscription: %w", err)
	}
	...
	fmt.Printf("\nCopilot authentication successful for user: %s\n", authBundle.Username)
