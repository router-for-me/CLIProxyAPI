...
func TestOpenAIModels_AfterRegisterCopilotAuth_Visible(t *testing.T) {
	...
	if _, err := cliproxy.NewBuilder().WithConfig(cfg).WithConfigPath(filepath.Join(tmp, "config.yaml")).WithCoreAuthManager(mgr).Build(); err != nil {
	...
	if _, err := mgr.Register(context.Background(), a); err != nil {
	...
	cliproxy.GlobalModelRegistry().RegisterClient(a.ID, "copilot", registry.GetCopilotModels())
	...
	for _, item := range out.Data {
		...
		if id == "gpt-5-mini" {
			if owner, _ := item["owned_by"].(string); owner != "copilot" {
				t.Fatalf("expected gpt-5-mini owned_by copilot, got %q", owner)
			}
		}
	}
	for id, seen := range required {
		if !seen {
			t.Fatalf("expected copilot model %s to be registered", id)
		}
	}
}
